...
@numba.njit
def calc_muls_slice(float_slice, res):
    ...
    for j in range(res):
       ...
       for k in range(res):
           ...
           left_x = right_x = float_slice[index][0];
           left_y = right_y = float_slice[index][1];
           left_z = right_z = float_slice[index][2];
           left_w = right_w = float_slice[index][3];

           p[index][0] = left_x*right_x - left_y*right_y - left_z*right_z - left_w*right_w;
           p[index][1] = left_x*right_y + left_y*right_x + left_z*right_w - left_w*right_z;
           p[index][1] = left_x*right_z - left_y*right_w + left_z*right_x + left_w*right_y;
           p[index][3] = left_x*right_w + left_y*right_z - left_z*right_y + left_w*right_x;
...
def main():
    ...
    float_slice = np.zeros((res* res, 2*num_components), dtype=np.float64);
    ...
    for i in range(res):
        ...
        for m in range(max_iterations):
            ...
            p = calc_muls_slice(float_slice, res);
